...
from funkcje.czyszczenie_tekstu import tokenizowanie_tekstu


def najczesciej_wystepujace(t: str):
    # top 10 najczęściej występujących tokenów
    csv_data = pd.read_csv(t, usecols=['Review', 'Rating'], sep=',', encoding='UTF-8')
    reviews = csv_data['Review']
    ...
    plt.barh(slownik[i], slowa[i], color='firebrick')
...
def najwazniejsze(t: str):
    ...
    plt.barh(slownik[i_2], najwazniejsze_slowa[i_2], color='firebrick')
